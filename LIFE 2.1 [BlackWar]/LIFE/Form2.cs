...
namespace LIFE
    ...
    public partial class Form2: Form
        ...
        private void pictureBox1_Click (object sender , EventArgs e)
            {
            int x = Convert.ToInt32(Math.Truncate(Convert.ToDecimal(Cursor.Position.X- pictureBox1.Location.X-this.Location.X) / (Convert.ToDecimal(sizeOfWindow) / size)));
            int y = Convert.ToInt32(Math.Truncate(Convert.ToDecimal(Cursor.Position.Y - pictureBox1.Location.Y - this.Location.Y) / (Convert.ToDecimal(sizeOfWindow) / size)));
