...
namespace LIFE
    ...
    class ScannerTerrainDecorator: AbstractTerrainDecorator
        ...
        public override void MakeTurn ()
            ...
            terrain.MakeTurn();
            i = 0;
            while (i < listBCells.Count)
                {
                listBCells[i].MakeTurn();
                i++;
                }

            //terrain.Draw(f1.pictureBox1);
            BlackDraw(f1.pictureBox1);
        ...
        public void BlackDraw (PictureBox p)
            {
            AfterInBefore();
            Scan();
            ...
            for (int  x = 0 ; x<Form1.size ;x++)
                for (int y = 0 ; y < Form1.size ; y++)
                    if (arrayBCells[x,y])
                    g.FillRectangle(sbBlack, new Rectangle(new Point(Convert.ToInt32(x * img.Height / size) + 1 , Convert.ToInt32(y * img.Height / size) + 1) , new Size(Convert.ToInt32(img.Height / size) - 1 , Convert.ToInt32(img.Height / size) - 1)));
        ...
        private void FillCell (int x , int y)
            {
            if (endPat)
                listBCells.Add(new BlackCells());
            listBCells[listBCells.Count - 1].Add(x , y);
            arrayBCells[x , y] = true;

            Terrain.cells[x , y].stateAfter = false;
